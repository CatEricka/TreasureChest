...
public class MMOItemsSupport {
    ...
    /**
     * build MMOItem by MMOItems' type and id string
     *
     * @param type MMOItems' type string
     * @param id   MMOItems' id string
     * @return MMOItem, if type and id exist in MMOItems' config repo
     */
    public Optional<MMOItem> newMMOItem(@Nullable String type, @Nullable String id) {
        return Optional
                .ofNullable(plugin().getMMOItem(plugin().getType(type), id));
    }
    ...
    public Optional<ItemStack> buildItemStack(@NotNull MMOItemWrapper mmoItemWrapper) {
        return newMMOItem(mmoItemWrapper.getType(), mmoItemWrapper.getId())
                .map(mmoItem -> {
                    ItemStack item = mmoItem
                            .newBuilder()
                            .build();
